...
\subsection{RAG (Retrieval-Augmented Generation)}\footnote{Ce texte a en partie été généré par intelligence artificielle.}
\index{RAG}
Le RAG (Retrieval-Augmented Generation) est une technique qui consiste à enrichir le prompt d'un utilisateur avec des informations suplémentaires provenant de sources externes, afin de rendre la réponse du LLM plus précise et pertinente.

Un RAG est crée via l'indexation de documents, qui sont ensuite stockés dans une base de données vectorielle. Cette étape d'indexation prend plus ou moins de temps en fonction du nombre de documents à indexer, mais aussi du matériel utilisé. Une fois les documents indexés, il est possible de faire des recherches dans la base de données vectorielle pour trouver les documents les plus pertinents en fonction d'une requête utilisateur. Ces documents sont ensuite utilisés pour enrichir le prompt envoyé au LLM, ce qui permet d'obtenir une réponse plus précise et adaptée au contexte.

Dans le contexte de ce TB, le RAG doit servir à stocker les informations sur la machine industrielle. Que ce soit les documentations techniques (datasheet) de chaque composant, la CAO (Conception Assistée par Ordinateur) de la machine, ou encore le code source du programme de contrôle de la machine. Toutes ces informations sont cruciales pour permettre au LLM d'avoir un contexte complet sur la machine, afin de pouvoir correctement répondre aux requêtes des utilisateurs et proposer des solutions adaptées en cas de problème. En intégrant un RAG dans l'architecture du système, nous pouvons garantir que le LLM dispose de toutes les informations nécessaires pour comprendre et agir efficacement dans un environnement industriel complexe.